...
FEATURED_WORKS = [
    {
        ...
        "label": "공식 파티챗",
        "genre": "법정 코미디 드라마",
        "summary": "민트초코의 존폐를 두고 4인이 실시간으로 입장을 정리하면 AI가 법정 공방과 판결문을 생성합니다.",
    ...
    },
    {
        ...
        "label": "시네마틱 스토리",
        "genre": "문명 시뮬레이션",
        "summary": "기원전 4000년의 첫 도시를 설계하면서 선택이 누적되고, 기억 시스템이 세계관을 계속 재구성합니다.",
    ...
    },
    {
        ...
        "label": "캐릭터 프리미엄",
        "genre": "SF 로맨스",
        "summary": "감정 곡선과 친밀도 단계에 따라 말투와 기억이 바뀌는 고급 캐릭터 채팅 패키지입니다.",
        "tags": ["하이퍼챗", "친밀도", "팬클럽"],
        "metrics": {
            "concurrent": "92K 활성 팬",
    ...
    },
    {
        ...
        "label": "크리에이터 히트작",
        "genre": "무협 성장",
        "summary": "이미지 300장 이상과 분기 서사를 결합한 장기 성장형 IP로, 창작자 수익화 사례를 상징합니다.",
...
STORY_EPISODES = [
    {
        "id": "arrival",
        "title": "포털 진입",
        "scene": "네온 빛으로 갈라진 항구 도시 위로 포털이 열리고, 당신의 이름이 아직 기록되지 않은 왕좌 지도가 내려옵니다.",
        "choices": [
            {
                "id": "negotiate",
                "label": "항만 길드와 협상한다",
    ...
    },
    {
        "id": "ascend",
        "title": "왕좌 점화",
        "scene": "모든 세력이 광장에 집결합니다. 마지막 선언이 도시의 장르를 뒤집습니다.",
        "choices": [
            {
                "id": "mercy",
            ...
            },
            {
                "id": "crown",
                "label": "왕좌를 강제로 점화한다",
                "result": "폭발 엔딩. 도시 전체가 당신을 중심으로 재편됩니다.",
                "trust_delta": -6,
                "hype_delta": 24,
            },
            {
...
PARTY_SCENARIOS = {
    "trial": {
        "id": "trial",
        "title": "민트초코 공판 시즌 2",
        "premise": "네 명의 플레이어가 맛과 기억, 자유의 권리를 두고 법정에서 실시간으로 공방합니다.",
        "player_roles": ["변호사", "심리 분석가", "셰프", "배심원 인플루언서"],
        "twists": [
            "판사가 AI였다는 사실이 드러납니다.",
            "방청석 팬덤 투표가 판결 점수에 반영됩니다.",
            "마지막 30초에 새 증인이 입장합니다.",
        ],
    },
    "ark": {
        "id": "ark",
        "title": "스페이스 오디세이: 산소 17분",
        "premise": "우주선의 남은 산소는 17분. 모두가 동시에 행동을 제출하면 AI가 다음 장면과 생존률을 계산합니다.",
        "player_roles": ["항해사", "의무관", "기관사", "언어학자"],
        "twists": [
            "구조 신호가 사실은 함정일 수 있다는 로그가 열립니다.",
            "선체 일부가 기억을 가진 생명체였다는 암시가 나옵니다.",
            "희생 없이 모두를 살리는 제5의 선택지가 발견됩니다.",
        ],
    },
}
...
def list_feed(query: str | None = None) -> list[dict[str, Any]]:
    ...
    return [
        ...
        if normalized
        in " ".join([work["title"], work["genre"], work["summary"], " ".join(work["tags"])]).casefold()
    ]


def get_feed_item(work_id: str) -> dict[str, Any] | None:
    return next((work for work in FEATURED_WORKS if work["id"] == work_id), None)


def advance_story(episode_id: str, choice_id: str, trust_score: int, hype_score: int) -> dict[str, Any] | None:
    episode_index = next(
        (index for index, episode in enumerate(STORY_EPISODES) if episode["id"] == episode_id),
        None,
    )
    ...
    choice = next(
        (entry for entry in STORY_EPISODES[episode_index]["choices"] if entry["id"] == choice_id),
        None,
    )
    if choice is None:
        return None

    next_episode = STORY_EPISODES[min(episode_index + 1, len(STORY_EPISODES) - 1)]
    return {
        "title": choice["label"],
...
def build_chat_reply(character_id: str, message: str, turn_index: int) -> dict[str, str] | None:
    ...
    responses = character["responses"][tone]
    return {
        "character_id": character_id,
        "character_name": character["name"],
        "reply": responses[turn_index % len(responses)],
        "tone": tone,
    }


def resolve_party_action(scenario_id: str, action: str, turn_index: int) -> dict[str, str] | None:
    scenario = PARTY_SCENARIOS.get(scenario_id)
    if scenario is None:
        return None
...
def generate_image_shot(prompt: str, style_id: str, index: int) -> dict[str, str] | None:
    style = next((entry for entry in IMAGE_STYLES if entry["id"] == style_id), None)
    if style is None:
        return None
